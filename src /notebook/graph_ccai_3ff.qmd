---
title: "Présentation des résultats : CCAI avec le modèle à 3 facteurs"
format: html
editor: visual
...
source("../panel_ccai_3ff.R")
```

On s'intéresse dans la suite à mesurer la contribution de $CCAI$ dans l'explication des rendements d'actions du S&P 500.
...
\begin{multline}
    ...
    +\beta_{CCAI}.CCAI_t + \eta_s + \eta_i+\varepsilon_{i,t}
\end{multline}
```
Tableau :
...
\begin{multline}
    r_{i,t} - r_t^f = \alpha + \beta_{MKT} (r_t^m - r_t^f) + \beta_{SMB} SMB_t + \beta_{HML} HML_t \\ 
    + \sum_{s=1}^S \beta_{CCAI}^s \mathbb{1}_{i \in s} CCAI_t + \eta_i + \varepsilon_{i,t}
\end{multline}
```
...
```{=tex}
\begin{multline}    
    r_{i \in t} - r_t^f = \alpha + \beta_{MKT} (r_t^m - r_t^f) + \beta_{SMB} SMB_t + \beta_{HML} HML_t  + \beta_{CCAI} CCAI_t +  \\
    \mathbb{1}_{t > PA} \left( \hat{\alpha} + \hat{\beta}_{MKT} (r_t^m - r_t^f) + \hat{\beta}_{SMB} SMB_t + \hat{\beta}_{HML} HML_t + \hat{\beta}_{CCAI} CCAI_t \right) + \eta_s + \eta_i + \varepsilon_{i,t}
\end{multline}
```
...
```{=tex}
\begin{multline}    
    r_{i \in t} - r_t^f = \alpha + \beta_{MKT} (r_t^m - r_t^f) + \beta_{SMB} SMB_t + \beta_{HML} HML_t  + \sum_{s=1}^S \beta_{CCAI}^s \mathbb{1}_{i \in s}CCAI_t +  \\
    \mathbb{1}_{t > PA} \left( \hat{\alpha} + \hat{\beta}_{MKT} (r_t^m - r_t^f) + \hat{\beta}_{SMB} SMB_t + \hat{\beta}_{HML} HML_t + \sum_{s=1}^S \hat{\beta}_{CCAI}^s \mathbb{1}_{i \in s} CCAI_t \right) + \eta_i + \varepsilon_{i,t}
\end{multline}
```
...
\begin{multline}    
    r_{i \in s, t} - r_t^f = \alpha + \beta_{MKT}^s (r_t^m - r_t^f) + \beta_{SMB}^s SMB_t + \beta_{HML}^s HML_t  
     + \beta_{CCAI}^s CCAI_t + \eta_i + \varepsilon_{i,t}
\end{multline}
```
...
```{=tex}
\begin{multline}    
    r_{i \in s, t} - r_t^f = \alpha + \beta_{MKT}^s (r_t^m - r_t^f) + \beta_{SMB}^s SMB_t + \beta_{HML}^s HML_t +  \beta_{CCAI}^s CCAI_t +  \\
    \mathbb{1}_{t > PA} \left( \hat{\alpha} + \hat{\beta}_{MKT}^s (r_t^m - r_t^f) + \hat{\beta}_{SMB}^s SMB_t + \hat{\beta}_{HML}^s HML_t + \hat{\beta}_{CCAI}^s CCAI_t \right) + \eta_i + \varepsilon_{i,t}
\end{multline}
```
