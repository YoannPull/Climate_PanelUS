...
# plot(zoo_ccai, main = "Série temporelle de ccai (fréquence hebdomadaire)",
#      xlab = "Date", ylab = "ccai")
...
data_variables$ccai <- as.numeric(data_variables$ccai)
...
X_panel[, `:=`(
  ccai_SectorConsumer.Discretionary = ccai * `SectorConsumer Discretionary`,
  ccai_SectorConsumer.Staples       = ccai * `SectorConsumer Staples`,
  ccai_SectorEnergy                 = ccai * `SectorEnergy`,
  ccai_SectorFinancials             = ccai * `SectorFinancials`,
  ccai_SectorHealth.Care            = ccai * `SectorHealth Care`,
  ccai_SectorIndustrials            = ccai * `SectorIndustrials`,
  ccai_SectorInformation.Technology = ccai * `SectorInformation Technology`,
  ccai_SectorMaterials              = ccai * `SectorMaterials`,
  ccai_SectorReal.Estate            = ccai * `SectorReal Estate`,
  ccai_SectorUtilities              = ccai * `SectorUtilities`
)]
...
X_panel[, `:=`(
  ...
  post_break_ccai = post_break * ccai
)]
...
# Note : Les coefficients rmw et cma ne sont pas significatif,  
# on va donc les retirer pour la suite

# Modèle avec effets fixes individuels et sectoriels
model <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + ccai +
               SectorConsumer.Discretionary + SectorConsumer.Staples + 
               SectorEnergy + SectorFinancials + SectorHealth.Care + 
               SectorIndustrials + SectorInformation.Technology + 
               SectorMaterials + SectorReal.Estate + SectorUtilities,
...
#                    rmw_5_friday + cma_5_friday + ccai + ticker,
#                  data = X_panel)
...
# Formuler le modèle
model_formula <- r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + ccai +
...
for (sector in unique_sector) {
  ...
  coefficients_before <- coef_test["ccai", "Estimate"]
  coefficients_after <- coef_test["post_PA_ccai", "Estimate"]
  p_values_before <- coef_test["ccai", "Pr(>|t|)"]
  p_values_after <- coef_test["post_PA_ccai", "Pr(>|t|)"]
  data_metrics   <- extract_metrics(model) 
...
# Créer le modèle de régression à effets fixes incluant les interactions
model <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly +  ccai +
               post_PA_mkt_rf_3_weekly + post_PA_smb_3_weekly + 
               post_PA_hml_3_weekly + post_PA_ccai +
               SectorConsumer.Discretionary + SectorConsumer.Staples + 
               SectorEnergy + SectorFinancials + SectorHealth.Care + 
               SectorIndustrials + SectorInformation.Technology + 
               SectorMaterials + SectorReal.Estate + SectorUtilities,
...
coefficients <- coef_test[c("ccai", "post_PA_ccai"), "Estimate"]
p_values <- coef_test[c("ccai", "post_PA_ccai"), "Pr(>|t|)"]
data_metrics <- extract_metrics(model)
data_metrics

# Créer un dataframe pour le plot
results_df <- data.frame(
  Coefficient = c("ccai", "post_PA_ccai"),
...
graphique <- ggplot(results_df, aes(x = Coefficient, y = Estimated_Value, fill = Significance)) +
  ...
  labs(x = "Secteur", 
       y = "Coefficient estimé",
       title = "Coefficients du panel pour ccai et post_PA_ccai",
       subtitle = "Avec indication de la significativité (P-value < 0.1)")  +
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
...
predictor <- "ccai"  # Nom du prédicteur principal
...
for (sector in unique_sector) {
  ...
  model_3f <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + ccai,
                  data = pdata_temp, model = "within",effect = "individual")
  
  model_5f <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + rmw_5_friday + cma_5_friday + ccai,
                  data = pdata_temp, model = "within",effect = "individual")
...
# # Affichage de la série temporelle
# plot(zoo_ccai_ar, main = "Série temporelle de ccai (fréquence hebdomadaire)",
#      xlab = "Date", ylab = "ccai")
...
for (sector in unique_sector) {
  ...
  # Extraire les coefficients avant et après les accords de Paris
  coefficients_before <- coef_test["ccai", "Estimate"]
  coefficients_after <- coef_test["post_break_ccai", "Estimate"]
  p_values_before <- coef_test["ccai", "Pr(>|t|)"]
  p_values_after <- coef_test["post_break_ccai", "Pr(>|t|)"]
