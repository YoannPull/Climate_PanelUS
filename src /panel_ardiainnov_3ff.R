...
# Ajout des résidus de MCCC dans le data frame data_variables
data_variables$mccc_innovation <- residuals
data_variables$mccc_innovation <- as.numeric(data_variables$mccc_innovation)
...
X_panel <- merge(data_temp, data_variables[,.(date, rf_3_weekly, mkt_rf_3_weekly, smb_3_weekly, hml_3_weekly,
                                              rmw_5_friday, cma_5_friday,mccc,mccc_innovation, ccai)],
                 by = "date", all.x = TRUE)
X_panel[, r_rf := r_stock - rf_3_weekly]
X_panel <- X_panel[, .(ticker,date, r_rf, mkt_rf_3_weekly, smb_3_weekly, hml_3_weekly,
                       rmw_5_friday, cma_5_friday, mccc, mccc_innovation, ccai, Sector,
...
X_panel[, `:=`(
  mccc_SectorConsumer.Discretionary = mccc_innovation * `SectorConsumer Discretionary`,
  mccc_SectorConsumer.Staples       = mccc_innovation * `SectorConsumer Staples`,
  mccc_SectorEnergy                 = mccc_innovation * `SectorEnergy`,
  mccc_SectorFinancials             = mccc_innovation * `SectorFinancials`,
  mccc_SectorHealth.Care            = mccc_innovation * `SectorHealth Care`,
  mccc_SectorIndustrials            = mccc_innovation * `SectorIndustrials`,
  mccc_SectorInformation.Technology = mccc_innovation * `SectorInformation Technology`,
  mccc_SectorMaterials              = mccc_innovation * `SectorMaterials`,
  mccc_SectorReal.Estate            = mccc_innovation * `SectorReal Estate`,
  mccc_SectorUtilities              = mccc_innovation * `SectorUtilities`
)]
...
X_panel[, `:=`(
  ...
  post_PA_mccc = post_PA * mccc_innovation
)]
...
X_panel[, `:=`(
  ...
  post_break_mccc = post_break * mccc_innovation
)]
...
model <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + 
               rmw_5_friday + cma_5_friday + mccc_innovation +
...
for (sector in unique_sector) {
  ...
  coefficients_before <- coef_test["mccc_innovation", "Estimate"]
  coefficients_after <- coef_test["post_PA_mccc", "Estimate"]
  p_values_before <- coef_test["mccc_innovation", "Pr(>|t|)"]
...
graphique <- ggplot(results_df, aes(x = Secteur, y = Coefficient, fill = Pvalue <= 0.1)) +
  ...
  labs(x = "Secteur", 
       y = "Coefficient estimé",
       title = "Coefficients du panel global par mccc_innovation*Secteur",
       subtitle = "Avec indication de la significativité (P-value < 0.1)") +
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
...
coefficients <- coef_test[c("mccc_innovation", "post_PA_mccc"), "Estimate"]
pvalues <- coef_test[c("mccc_innovation", "post_PA_mccc"), "Pr(>|t|)"]
data_metrics <- extract_metrics(model)
data_metrics

# Créer un dataframe pour le plot
results_df <- data.frame(
  Coefficient = c("mccc_innovation", "post_PA_mccc"),
...
predictor <- "mccc_innovation"  # Nom du prédicteur principal
...
for (sector in unique_sector) {
  ...
  model_3f <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + mccc_innovation,
                  data = pdata_temp, model = "within",effect = "individual")
  
  model_5f <- plm(r_rf ~ mkt_rf_3_weekly + smb_3_weekly + hml_3_weekly + mccc_innovation,
                  data = pdata_temp, model = "within",effect = "individual")
...
# mccc
data_temp <- copy(data_variables[,.(date,mccc_innovation)])
data_temp$date <- as.Date(data_temp$date)

# Création d'une série temporelle zoo
zoo_mccc_innovationi_ar <- zoo(data_temp$mccc_innovation, order.by = data_temp$date)

summary(ur.za(zoo_mccc_innovationi_ar, model="both",lag=1))

zoo_mccc_innovationi_ar[696]

# plot(ur.za(zoo_mccc_innovationi_ar, model="both",lag=1))

# Affichage de la série temporelle
# plot(zoo_mccc_innovationi_ar, main = "Série temporelle de mccc_innovation (fréquence hebdomadaire)",
#      xlab = "Date", ylab = "mccc_innovation")
# abline(v = index(zoo_mccc_innovationi_ar)[696], col = "red", lwd = 1)
